...
class CountParameters(object):
    ...
    def write_results(self, results):
        """
        Function to write the read count's results while appending some
        parameters.
        Read count's results are basically the results generated by the
        function run_serial_count_reads or run_parallel_count_reads in
        count_reads_in_feature/core.py.

        This function basically call the _write_output function in the utils.py
        script containing utility functions used by many.
        ...
        _write_output(
            ...
            self.output_append,
            sparse = self.counts_output_sparse,
            dtype = np.float32,
        )
