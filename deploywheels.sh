...
HTSEQ_WHEEL_FILE=$(ls /io/wheelhouse/*.whl | grep -i htseq)
...
for PYBIN in ${PYBINS}; do
  ...
  ${PYBIN}/twine upload --verbose --repository-url "${TWINE_REPOSITORY}" -u "${TWINE_USERNAME}" -p "${TWINE_PASSWORD}" ${HTSEQ_WHEEL_FILE}
  if [ $? != 0 ]; then
    ERRS=1
  fi
done
