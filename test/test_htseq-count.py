...
scripts_folder = conftest.get_scripts_folder()
...
class HTSeqCount(HTSeqCountBase):
    cmd = scripts_folder + "htseq-count"

    def test_version(self):
        self._run(
            {
                "call": [self.cmd, "--version"],
            }
        )
    ...
    def test_output_tsv_header(self):
        ...
        self._run(
            {
                "call": [
                    ...
                    " ",
                    "-c",
                    f"{data_folder}/10x_pbmc1k/test_tsv_header.tsv",
        ...
        try:
            dat = pd.read_csv(
                f"{data_folder}/10x_pbmc1k/test_tsv_header.tsv", delimiter="\t"
            )
    ...
    def test_additional_attributes_h5ad(self):
        ...
        self._run(
            {
                "call": [
                    ...
                    "exon_number",
                    "-c",
                    f"{data_folder}/10x_pbmc1k/subsampled_with_missing_barcodes_counts.h5ad",
        ...
        try:
            ...
            # clean up
            os.remove(
                data_folder + "/10x_pbmc1k/subsampled_with_missing_barcodes_counts.h5ad"
            )
    ...
    def test_no_contig_overlap_warning(self):
        ...
        stderr_template = open(
            f"{data_folder}/no_contig_overlap_warning_stderr.txt"
        ).read()
    ...
    def test_contig_overlap_no_warning(self):
        ...
        expected_stderr = open(
            f"{data_folder}/contig_overlap_no_warning_stderr.txt"
        ).read()
        self._run(
            {
                "call": [
...
class HTSeqCountBarcodes(HTSeqCountBase):
    cmd = scripts_folder + "htseq-count-barcodes"
    ...
    def test_missing_barcodes(self):
        ...
        self._run(
            {
                "call": [
                    ...
                    "-c",
                    f"{data_folder}/10x_pbmc1k/subsampled_with_missing_barcodes_counts.csv",
                    "--samout",
                    f"{data_folder}/10x_pbmc1k/subsampled_with_missing_barcodes_counts.sam",
                    f"{data_folder}/10x_pbmc1k/subsampled_with_missing_barcodes.sam",
                    f"{data_folder}/10x_pbmc1k/HomoSapiens.GRCh38-2020-A_subsampled.gtf",
                ],
        ...
        try:
            df = pd.read_csv(
                data_folder + "/10x_pbmc1k/subsampled_with_missing_barcodes_counts.csv",
                header=None,
                delimiter="\t",
            )
            df.columns = ["gene_ids", "count"]
            self.assertEqual(
                int(df.loc[df["gene_ids"] == "ENSG00000188976"]["count"].iloc[0]), 1
            )
            self.assertEqual(
                int(df.loc[df["gene_ids"] == "ENSG00000251562"]["count"].iloc[0]), 1
            )
            ...
            samfile = pysam.AlignmentFile(
                data_folder + "/10x_pbmc1k/subsampled_with_missing_barcodes_counts.sam"
            )
            read_assignments = {
